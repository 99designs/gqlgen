...
| Computed | `computed_requires` | `@goComputed` (per field) | Delivered to standalone field resolvers via a `federationRequires` argument (Federation 2 only). |
...
per field, by `@goComputed`.) Because `@entityResolver` is your own directive, add
the `requires: String` argument to its definition. With no `requires:` argument,
an entity falls back to the package default.

#### Computing individual `@requires` fields with `@goComputed`

`computed_requires` computes **every** `@requires` field on its entities. To
compute just **one** field, annotate it with `@goComputed` (a gqlgen-provided
...
type Product @key(fields: "id") @entityResolver(multi: true) {
  ...
  summary: String! @requires(fields: "info { label }") @goComputed  # computed field resolver
}
```

`@goComputed` requires Federation 2 and `call_argument_directives_with_null`. It
