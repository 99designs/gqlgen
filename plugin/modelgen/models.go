...
func (m *Plugin) MutateConfig(cfg *config.Config) error {
	...
	for _, it := range b.Scalars {
		cfg.Models.Add(it, "github.com/john-markham/gqlgen/graphql.String")
	}
...
func (m *Plugin) generateField(
	...
	if f.Omittable {
		...
		omittableType, err := binder.FindTypeFromName("github.com/john-markham/gqlgen/graphql.Omittable")
		if err != nil {
			return nil, err
		}
...
func findAndHandleCyclicalRelationships(b *ModelBuild) {
	for ii, structA := range b.Models {
		for _, fieldA := range structA.Fields {
			...
			// the field Type string will be in the form "github.com/john-markham/gqlgen/codegen/testserver/followschema.LoopA"
