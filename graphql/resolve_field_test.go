...
func TestResolveFieldRuntimeNonNull(t *testing.T) {
	t.Run(
		"should return RequiredNull when MarkNonNull is called and resolver returns nil",
		func(t *testing.T) {
			ctx := WithResponseContext(context.Background(), DefaultErrorPresenter, nil)
			oc := &OperationContext{
				ResolverMiddleware: func(ctx context.Context, next Resolver) (res any, err error) {
	...
	t.Run(
		"should return RequiredNull when MarkNonNull is called and resolver returns error",
		func(t *testing.T) {
			ctx := WithResponseContext(context.Background(), DefaultErrorPresenter, nil)
			oc := &OperationContext{
				ResolverMiddleware: func(ctx context.Context, next Resolver) (res any, err error) {
			...
			assert.Same(
				...
				RequiredNull,
				result,
				"should return RequiredNull when resolver errors with MarkNonNull",
			)
		},
	)

	t.Run(
		"should return normal result when MarkNonNull is called and resolver succeeds",
		func(t *testing.T) {
			ctx := WithResponseContext(context.Background(), DefaultErrorPresenter, nil)
			oc := &OperationContext{
				ResolverMiddleware: func(ctx context.Context, next Resolver) (res any, err error) {
			...
			assert.Equal(
				...
				`"hello"`,
				sb.String(),
				"successful resolve should return marshaled value, not a null sentinel",
			)
		},
	)
	...
	t.Run(
		"should return Null when both static nonNull and MarkNonNull are set",
		func(t *testing.T) {
			ctx := WithResponseContext(context.Background(), DefaultErrorPresenter, nil)
			oc := &OperationContext{
				ResolverMiddleware: func(ctx context.Context, next Resolver) (res any, err error) {
			...
			assert.Same(
				...
				Null,
				result,
				"static nonNull should return Null (existing template handles Invalids)",
			)
		},
	)
	...
	t.Run("MarkNonNull should work when called from middleware chain", func(t *testing.T) {
		...
		assert.Same(
			...
			RequiredNull,
			result,
			"MarkNonNull from ResolverMiddleware should trigger RequiredNull",
		)
	})
}
