...
func (ec *executionContext) resolveEntity(
	...
	case "PlanetRequiresNested":
		...
		case "findPlanetRequiresNestedByName":
			...
			entity.World.Foo, err = unmarshalNString2string(ctx, ec, rep["world"].(map[string]interface{})["foo"])
			if err != nil {
				return nil, err
			}
			return entity, nil
	...
	case "World":
		...
		case "findWorldByHelloNameAndFoo":
			id0, err := unmarshalNString2string(ctx, ec, rep["hello"].(map[string]interface{})["name"])
			if err != nil {
...
func (ec *executionContext) resolveManyEntities(
	...
	case "MultiPlanetRequiresNested":
		...
		case "findManyMultiPlanetRequiresNestedByNames":
			...
			for i, entity := range entities {
				entity.World.Foo, err = unmarshalNString2string(ctx, ec, reps[i].entity["world"].(map[string]interface{})["foo"])
				if err != nil {
					return err
				}
				list[reps[i].index] = entity
