...
- **Four `@requires` strategies**: `default`, `explicit`, `preloaded`, and
  ...
  `computed_requires` package option (all fields) or, per field, by `@computedRequires`
...
`computed_requires` package option (all fields) or `@computedRequires` (per field),
rather than the `@entityResolver(requires:)` directive.

### Per-field `computed` (`@computedRequires`)

`computed_requires` computes every `@requires` field on its entities; `@computedRequires`
...
resolver would otherwise handle it). `@computedRequires` requires Federation 2 +
...
`plugin/federation/testdata/perfieldcomputed`.
...
field `@computedRequires` to route just that field to a standalone field resolver
...
| `computed` selected by the package option, not the `requires:` directive | It routes fields off the entity resolver, so it is off the axis the directive names. Keeps that axis coherent (three members) and leaves no unreleased directive value to deprecate when per-field `@computedRequires` lands. |
...
- **Object/nested `@requires` for `preloaded`**, which needs gqlgen to
  ...
  such a field `@computedRequires` (see *Per-field `computed`*) to route it to a field
