...
func TestSubscription(t *testing.T) {
	...
	createdAt := "2021-01-01"
	resolvers.SubscriptionResolver.UserCreated = func(ctx context.Context) (<-chan graphql.Event[*User], error) {
		ch := make(chan graphql.Event[*User], 1)
		go func() {
			defer close(ch)
			ch <- graphql.Event[*User]{Context: ctx, Value: &User{
				ID:        "1",
				Name:      "testUser",
				Email:     "testEmail",
				Age:       nil,
...
func TestSubscriptionContextField(t *testing.T) {
	...
	// AroundResponses receives the per-event context as its ctx argument.
	srv.AroundResponses(func(ctx context.Context, next graphql.ResponseHandler) *graphql.Response {
		if id, ok := ctx.Value(eventKey).(string); ok {
			capturedEventID = id
		}
		return next(ctx)
	})
	...
	resolvers.SubscriptionResolver.UserCreated = func(ctx context.Context) (<-chan graphql.Event[*User], error) {
		...
		go func() {
			...
			ch <- graphql.Event[*User]{Context: eventCtx, Value: &User{
				ID:        "1",
				Name:      "testUser",
				Email:     "testEmail",
				Age:       nil,
	...
	t.Run("per-event context propagates to AroundResponses", func(t *testing.T) {
		var resp struct {
			UserCreated struct {
				ID string `json:"id"`
			} `json:"userCreated"`
		...
		require.Equal(t, "event-42", capturedEventID,
			"per-event context from graphql.Event must propagate to AroundResponses")
	})
}
