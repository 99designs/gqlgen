...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Planet":
		resolverName, err := entityResolverNameForPlanet(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Planet": %w`, err)
		}
		switch resolverName {
		...
		case "findPlanetByName":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPlanetByName(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindPlanetByName(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Planet": %w`, err)
			}
...
func (ec *executionContext) resolveManyEntities(
	...
	case "Product":
		...
		case "findManyProductByIDs":
			typedReps := make([]*model.ProductByIDsInput, len(reps))
			...
			for i, rep := range reps {
				...
				representation := &model.ProductByIDsInput{
					ID: id0,
				}
				representation.Category, err = ec.unmarshalNString2string(ctx, rep.entity["category"])
				if err != nil {
					return err
				}

				typedReps[i] = representation
			}
...
func entityResolverNameForPlanet(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"name\" for Planet", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Planet", ErrTypeNotFound))
			break
		}
		return "findPlanetByName", nil
	}
	return "", fmt.Errorf("%w for Planet due to %v", ErrTypeNotFound,
...
func entityResolverNameForProduct(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		return "findManyProductByIDs", nil
	}
