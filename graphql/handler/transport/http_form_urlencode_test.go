...
func TestUrlEncodedForm(t *testing.T) {
	...
	t.Run("decode failure json", func(t *testing.T) {
		...
		assert.Equal(t, `{"errors":[{"message":"Unexpected Name \"notjson\"","locations":[{"line":1,"column":1}],"extensions":{"code":"GRAPHQL_PARSE_FAILED"}}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("decode failure urlencoded", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusUnprocessableEntity, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `{"errors":[{"message":"Expected Name, found \u003cInvalid\u003e","locations":[{"line":1,"column":6}],"extensions":{"code":"GRAPHQL_PARSE_FAILED"}}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("parse query failure", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusUnprocessableEntity, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, resp.Body.String(), `{"errors":[{"message":"could not cleanup body: json: cannot unmarshal object into Go struct field RawParams.query of type string"}],"data":null,"hasNext":false}`)
	})
