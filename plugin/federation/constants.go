...
// The @goComputed directive marks a single @requires field to be delivered via a
// standalone field resolver (the computed strategy) instead of through the entity
// resolver, so one entity can mix computed and non-computed @requires fields.
const dirNameGoComputed = "goComputed"
...
const federationVersion1Schema = `
	...
	directive @goComputed on FIELD_DEFINITION
	scalar _Any
	scalar _FieldSet
`
