...
func TestLoadSchema(t *testing.T) {
	...
	t.Run("with valid operations", func(t *testing.T) {
		cfg := getConfig(t)
		...
		cfg.OperationSources = []string{
			"query { message { id } }",
		}
		err := cfg.LoadSchema()
		require.NoError(t, err)
		require.NotNil(t, cfg.Operations)
	})

	t.Run("with invalid operations", func(t *testing.T) {
		cfg := getConfig(t)
		...
		cfg.OperationSources = []string{
			"query { aThing { thatDoestExist }",
		}
		err := cfg.LoadSchema()
		require.Error(t, err)
		require.Nil(t, cfg.Operations)
	})
