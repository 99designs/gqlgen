...
func TestPOST(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, `{"data":{"name":"test"},"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("decode failure", func(t *testing.T) {
		...
		assert.Equal(t, `{"errors":[{"message":"json request body could not be decoded: invalid character 'o' in literal null (expecting 'u') body:notjson"}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("parse failure", func(t *testing.T) {
		...
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `{"errors":[{"message":"Unexpected !","locations":[{"line":1,"column":1}],"extensions":{"code":"GRAPHQL_PARSE_FAILED"}}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("validation failure", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusUnprocessableEntity, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `{"errors":[{"message":"Cannot query field \"title\" on type \"Query\".","locations":[{"line":1,"column":3}],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("invalid variable", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusUnprocessableEntity, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `{"errors":[{"message":"cannot use bool as Int","path":["variable","id"],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("execution failure", func(t *testing.T) {
		...
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `{"errors":[{"message":"mutations are not supported"}],"data":null,"hasNext":false}`, resp.Body.String())
	})
	...
	t.Run("validate content type", func(t *testing.T) {
		...
		for _, tc := range invalidContentTypes {
			t.Run(fmt.Sprintf("reject for content type %s", tc), func(t *testing.T) {
				...
				assert.Equal(t, http.StatusBadRequest, resp.Code, resp.Body.String())
				assert.Equal(t, fmt.Sprintf(`{"errors":[{"message":"%s"}],"data":null,"hasNext":false}`, "transport not supported"), resp.Body.String())
			})
		}
	})
}
