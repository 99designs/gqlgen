...
func writeContent(t *testing.T, filename, content string) {
	t.Helper()
...
//
// Exact POSIX permission bits (cases 1 & 2) are meaningless on Windows, which
// has no POSIX mode — os.FileMode there is a coarse read-only/not emulation,
// not real owner/group/other bits (see renameio's own writefile.go, which is
// `//go:build !windows` for the same reason). Those two assertions are
// skipped on windows; case 3 (no leftover temp file) is platform-agnostic and
// always runs.
func TestWriteIsAtomicPreservesPermissionsAndLeavesNoTemp(t *testing.T) {
	...
	if runtime.GOOS != "windows" {
		...
		// Still exercise the write path on windows (case 3 below checks its
		// leftover-temp-file property); just don't assert POSIX bits.
		writeContent(t, filepath.Join(dir, "existing.go"), goFile)
		writeContent(t, filepath.Join(dir, "fresh.go"), goFile)
	}
...
func TestWriteIsAtomicAndUnchangedShortCircuit(t *testing.T) {
	...
	// Exact POSIX mode is meaningless on windows (no real owner/group/other
	// bits) — see the note on TestWriteIsAtomicPreservesPermissionsAndLeavesNoTemp.
	if runtime.GOOS != "windows" {
