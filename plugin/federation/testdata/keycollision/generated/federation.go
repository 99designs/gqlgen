...
func (ec *executionContext) resolveManyEntities(
	...
	case "Collision":
		resolverName, err := entityResolverNameForCollision(ctx, reps[0].entity)
		if err != nil {
			return fmt.Errorf(`finding resolver for Entity "Collision": %w`, err)
		}
		switch resolverName {

		case "findManyCollisionByIDAndIDs":
			typedReps := make([]*model.CollisionByIDAndIDsInput, len(reps))
			...
			for i, rep := range reps {
				...
				id1, err := ec.unmarshalNString2string(ctx, rep.entity["i"].(map[string]any)["d"])
				if err != nil {
					return errors.New(fmt.Sprintf("Field %s undefined in schema.", "iD"))
				}

				typedReps[i] = &model.CollisionByIDAndIDsInput{
					ID:  id0,
					ID2: id1,
				}
			}
...
func entityResolverNameForCollision(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Collision", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"i\" for Collision", ErrTypeNotFound))
			break
		}
		if m, ok = val.(map[string]any); !ok {
			// nested field value is not a map[string]interface so don't use it
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to nested Key Field \"i\" value not matching map[string]any for Collision", ErrTypeNotFound))
			break
		}
		val, ok = m["d"]
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"d\" for Collision", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Collision", ErrTypeNotFound))
			break
		}
		return "findManyCollisionByIDAndIDs", nil
	}
	return "", fmt.Errorf("%w for Collision due to %v", ErrTypeNotFound,
