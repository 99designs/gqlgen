...
func (f *Federation) MutateConfig(cfg *config.Config) error {
	...
	cfg.Directives[dirNameGoComputed] = config.DirectiveConfig{SkipRuntime: true}
...
// entity is computed (via @goComputed or the computed_requires package option).
...
// validateComputedFields rejects unusable @goComputed placements up front: it is
// only meaningful on a @requires field, and it cannot be combined with the
...
func validateComputedFields(schemaType *ast.Definition, strategy RequiresStrategy) error {
	for _, field := range schemaType.Fields {
		if field.Directives.ForName(dirNameGoComputed) == nil {
			continue
		}
		if field.Directives.ForName(dirNameRequires) == nil {
			return fmt.Errorf(
				"entity %q field %q: @goComputed only applies to @requires fields",
				schemaType.Name, field.Name,
			)
		}
		if strategy == RequiresExplicit {
			return fmt.Errorf(
				"entity %q field %q: @goComputed cannot be combined with the explicit @requires strategy; the Populate%sRequires hook already handles every @requires field",
				schemaType.Name,
				field.Name,
				schemaType.Name,
			)
...
// package option), or the field is marked @goComputed. It is the single source of
// truth for the per-field computed decision, read by both buildRequires and
// mutateSchemaForRequires.
func requiresFieldIsComputed(strategy RequiresStrategy, field *ast.FieldDefinition) bool {
	return strategy == RequiresComputed ||
		field.Directives.ForName(dirNameGoComputed) != nil
}
