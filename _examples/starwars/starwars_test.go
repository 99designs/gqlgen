...
func TestStarwars(t *testing.T) {
	...
	t.Run("invalid enums in variables", func(t *testing.T) {
		...
		require.EqualError(t, err, `http 422: {"errors":[{"message":"INVALID is not a valid Episode","path":["variable","episode"],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null,"hasNext":false}`)
	})
